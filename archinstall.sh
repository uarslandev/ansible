...
if [[ "$FS_CHOICE" == "1" ]]; then
    ...
    if [[ "$ENABLE_ENC" =~ ^(y|yes)$ ]]; then
        # Use file:///dev/stdin during creation to avoid opening /dev/tty interactive prompt
        echo "$ZFS_PASS" | zpool create \
            "${POOL_OPTS[@]}" \
        ...
        # Set keylocation back to prompt for interactive login during system boot
    ...
    if [[ "$ENABLE_ENC" =~ ^(y|yes)$ ]]; then
        ...
        # Pass key using -L prompt override with stdin input
